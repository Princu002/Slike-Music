import slikeDispatcher from "./Dispatcher.js";
export default class slikeApi extends Map {
    client;
    constructor(client) {
        super();
        this.client = client;
    ...
    async handle(guild, member, channel, node, track) {
        ...
        if (!present) {
            ...
            const dispatcher = new slikeDispatcher(this.client, guild, channel, player);
    ...
    async reconnect(guild, vc, txt, node) {
        ...
        const dispatcher = new slikeDispatcher(this.client, guild, txt, player);
        this.set(guild.id, dispatcher);
        return dispatcher;
    }
}
