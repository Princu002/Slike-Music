...
export default class slikePlay extends slikeCommand {
    constructor(client) {
        super(client);
        this.name = "play";
        this.aliases = ["p"];
        ...
        this.exec = async (message, args, prefix) => {
            ...
            if (this.client.utils.checkUrl(query) === true) {
                if (query.match(this.client.spotify.spotifyPattern)) {
                    ...
                    let node = await this.client.spotify.nodes.get("LuxuryLabs");
                    ...
                    if (result.loadType === `PLAYLIST_LOADED`) {
                        ...
                        if (!dispatcher) {
                            ...
                            dispatcher = new slikeDispatcher(this.client, message.guild, message.channel, player);
            ...
            if (type === "direct") {
                ...
                let node = this.client.shoukaku.getNode();
                let b1 = this.client.utils.button(`custom_id`, null, 2, `slike_default_search`, null, this.client.emoji.defSearch);
                let b2 = this.client.utils.button(`custom_id`, null, 3, `slike_spoti_search`, null, this.client.emoji.spotiSearch);
                let buttons = [b1, b2];
                ...
                collector.on("collect", async (interaction) => {
                    if (interaction.customId === `slike_default_search`) {
                        let result = await node.rest.resolve(`ytsearch:${query}`);
                        if (!result.tracks.length)
                    ...
                    else if (interaction.customId === `slike_spoti_search`) {
                        let result;
                        this.client.kazagumo
                            ...
                            let track = result.tracks[0];
                            await this.client.spotify.requestToken();
                            let spoti = this.client.spotify.nodes.get("slike");
                    ...
                    else if (interaction.customId === `slike_deez_search`) {
                    ...
                    else if (interaction.customId === `slike_sound_search`) {
                        let node = this.client.shoukaku.getNode();
