...
export default class Queue extends slikeCommand {
    constructor(client) {
        super(client);
        this.name = "queue";
        this.aliases = ["q"];
        ...
        this.exec = async (message, args, prefix, dispatcher) => {
                ...
                return message.reply({
                    embeds: [
                        this.client.utils
                            ...
                            {
                                name: `Now Playing`,
                                value: `[${dispatcher.current.info.title.substring(0, 35)}](${this.client.config.voteUrl})`,
                            },
                        ])
            ...
            if (dispatcher.queue.length < 11) {
                return message.reply({
                    embeds: [
                        this.client.utils
                            ...
                            .setDescription(dispatcher.queue
                            .map((x, i) => `**${i + 1}.** [${x.info.title.substring(0, 40)}](${this.client.config.voteUrl}) [*${this.client.utils.humanize(x.info.length)}*]`)
                            .sort()
                            .join("\n"))
                            .setFooter({
                ...
                let queue = dispatcher.queue.map((x, i) => `**${i + 1}.** [${x.info.title.substring(0, 40)}](${this.client.config.voteUrl}) [*${this.client.utils.humanize(x.info.length)}*]`);
